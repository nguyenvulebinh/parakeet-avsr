"""Mel spectrogram feature extraction for AVASR.

Pure PyTorch/librosa implementation with no external ASR framework dependency.
"""
...
def normalize_batch(x, seq_len, normalize_type):
    ...
    if normalize_type == "per_feature":
        ...
        if (
            ...
            raise ValueError(
                "normalize_batch with `per_feature` received a length-1 tensor."
            )
...
class FilterbankFeatures(nn.Module):
    ...
    def __init__(
        ...
        if exact_pad and n_window_stride % 2 == 1:
            raise NotImplementedError("exact_pad requires even hop_size.")
        self.log_zero_guard_value = log_zero_guard_value
        if (
            n_window_size is None or n_window_stride is None
            or not isinstance(n_window_size, int) or not isinstance(n_window_stride, int)
            or n_window_size <= 0 or n_window_stride <= 0
        ):
            raise ValueError("n_window_size and n_window_stride must be positive ints.")
        logger.info("PADDING: %d", pad_to)
        ...
        if log_zero_guard_type not in ["add", "clamp"]:
            raise ValueError(f"Invalid log_zero_guard_type: {log_zero_guard_type}")
    ...
    def log_zero_guard_value_fn(self, x):
        if isinstance(self.log_zero_guard_value, str):
            if self.log_zero_guard_value == "tiny":
                ...
                raise ValueError(f"Unknown log_zero_guard_value: {self.log_zero_guard_value}")
        else:
...
class AudioToMelSpectrogramPreprocessor(nn.Module):
    """Mel-spectrogram preprocessor wrapping FilterbankFeatures."""
    ...
    def __init__(
        ...
        if window_size and n_window_size:
            raise ValueError("Specify only one of window_size or n_window_size.")
        if window_stride and n_window_stride:
            raise ValueError("Specify only one of window_stride or n_window_stride.")
        if window_size:
