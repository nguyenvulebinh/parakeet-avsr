...
def _fan_weights_file(filename: str) -> str:
    return str((get_model_bin_root() / "face_alignment" / "fan" / "weights" / filename).resolve())


def _fan_weight_path(filename: str) -> str:
    from src.avasr.io.face_model_hub import get_hf_hub_weight_path

    hub = get_hf_hub_weight_path("fan", filename)
    if hub is not None:
        return hub
    return _fan_weights_file(filename)
...
class FANPredictor(object):
    ...
    def get_model(name: str = '2dfan2') -> SimpleNamespace:
        name = name.lower()
        if name == '2dfan2':
            return SimpleNamespace(weights=_fan_weight_path("2dfan2.pth"),
            ...
            return SimpleNamespace(weights=_fan_weight_path("2dfan4.pth"),
            ...
            return SimpleNamespace(weights=_fan_weight_path("2dfan2_alt.pth"),
                                   config=SimpleNamespace(crop_ratio=0.55, input_size=256, num_modules=2,
                                                          hg_num_features=256, hg_depth=4, use_avg_pool=False,
                                                          use_instance_norm=False, stem_conv_kernel_size=7,
                                                          stem_conv_stride=2, stem_pool_kernel_size=2,
