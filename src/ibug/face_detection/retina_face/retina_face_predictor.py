...
def _retina_weights_file(filename: str) -> str:
    return str((get_model_bin_root() / "face_detection" / "retina_face" / "weights" / filename).resolve())


def _retina_weight_path(filename: str) -> str:
    from src.avasr.io.face_model_hub import get_hf_hub_weight_path

    hub = get_hf_hub_weight_path("retina", filename)
    if hub is not None:
        return hub
    return _retina_weights_file(filename)
...
class RetinaFacePredictor(object):
    ...
    def get_model(name: str = 'resnet50') -> SimpleNamespace:
        ...
        if name == 'resnet50':
            return SimpleNamespace(weights=_retina_weight_path("Resnet50_Final.pth"),
                                   config=SimpleNamespace(**deepcopy(cfg_re50)))
        elif name == 'mobilenet0.25':
            return SimpleNamespace(weights=_retina_weight_path("mobilenet0.25_Final.pth"),
